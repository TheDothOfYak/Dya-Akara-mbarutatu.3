...
- **The shared market — real players AND the Dya'kukull, no duplicates, no tell between them** — list a token to the online market and every player sees it; the full token travels with the listing. Buying is an **atomic conditional update**: exactly one buyer can ever win a token, and the moment they do it leaves the market for everyone else. The 100 Dya'kukull (deterministic ids `ai_0..ai_99` + `ai_elbergi`, identical on every device) list, buy, and haggle through this exact same path — nothing in the Browse screen distinguishes an AI seller from a real one. Offers/haggling are gold-only and version-gated so two devices answering the same due AI reply can't double-apply it. Cancelling and admin pulls return the token home. (`js/core/market_online.js`.)
- **Hard, real-time token supply caps** — the Admin Panel's **Token Limits** tab can cap how many of a given species (optionally per rarity tier) may ever exist. Every genuinely new token — hunting/crafting rewards, upgrades moving into a tier, admin grants, even the Dya'kukull's own ongoing hunting/crafting — reserves a slot from a live Supabase counter (`reserve_token_slot()`) *before* it's minted, in one atomic transaction, so two mints at the exact same instant can't both slip in under the cap. The Dya'kukull's original deterministic starting collections aren't counted (they're one fixed genesis population, not new production). (`dya_species_supply` in `supabase/schema.sql`.)
- **Every token, anywhere, in one admin view** — the **All Tokens** tab lists every token that exists (every real player's cloud collection plus all 100 AI), searchable and filterable, with a full editor: name, species, rarity, stats, status, frozen/flagged/rental, and reassigning the owner outright.
- **New Users review queue** — every new account is logged (unreviewed) with the tokens it currently holds; the admin can look it over and check it off in the **New Users** tab. It's a log, not a gate — the account plays completely normally whether or not, or however long before, anyone reviews it.
