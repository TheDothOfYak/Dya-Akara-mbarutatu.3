...
- **Brawl mode** — squad battles against the machine, playable on your own: **2v2**, **3v3**, **5v5** team battles and a **4-player free-for-all**. Like a duel there is no economy, pulse or Relic — you pick your side's fighters (only tokens that actually fight; short squads are topped up with minted allies so any collection can play any format), the machine fills every other side, and the whole field takes the ground at once. Combat is team-relative, so in the free-for-all every side fights for itself — the last team standing wins. The HUD shows a live per-team survivor count; a stalemate is called on total health remaining. Practice only: no XP, no rank, nothing staked. Rematch re-runs the same format and squad instantly.
