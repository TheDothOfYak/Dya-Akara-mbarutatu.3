...
- **Sandbox mode** — a testing ground in the Play tab: build **both** teams from tokens you own (no rentals), pick a terrain, then take either seat — **Control Team One**, **Control Team Two**, or **Watch (both AI)** with a ×1/×2/×4 speed toggle — to see how your creatures interact and fight each other. Nothing is recorded: no XP, no gold, no stats, no rank, so it can't be farmed. Rematch re-runs the same two teams instantly.
