...
- **Brawl mode** — a *normal* standard match (resources, pulses, Relics, deploy your tokens over the pulse) but with **more than two players**, each keeping their **own hoard and Relic** with their **name floating above their camp**. Three shapes, all vs the machine: **Team Battle** — allied sides at **2v2 / 3v3 / 5v5**, allies never fight each other and a side wins by carrying any rival Relic home or outlasting the rest (bases can sit apart down each flank, or clustered as one shared camp); **Free-for-all** — 3 or 4 players, everyone for themselves; **Surrounded** — you hold the centre of the arena, ringed by rivals who each play for themselves. Built on a new **alliance layer** (teams carry a `side`; targeting, friendly-fire, structures, zones, and the AI are all side-aware, so allies never harm each other while every rival side is a foe). The rival camps are seated from the world's Dya'kukull so their names are real. Practice only: no rank, no XP, nothing staked. Rematch re-runs the same setup. *(A 1v1 standard match is unchanged — `side` defaults to the team index, so two-team play is byte-identical.)*
